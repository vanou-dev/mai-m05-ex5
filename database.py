...
def get(protocol, subset, classes=CLASSES, variables=VARIABLES):
    ...
    ==========
      ...
      protocol : str
          one of the valid protocols supported by this interface

      subset : str
          one of 'train' or 'test'

      classes : :py:class:`list` of :py:class:`str`
          a list of strings containing the names of the classes from which you
          want to have the data from

      variables : :py:class:`list` of :py:class:`str`
          a list of strings containg the names of the variables (features) you
          want to have data from
    ...
    =======
      ...
      data : numpy.ndarray
          The data for all the classes and variables nicely packed into one numpy
          3D array. One depth represents the data for one class, one row is one
          example, one column a given feature.
